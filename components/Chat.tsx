...
export default function Chat() {
  ...
  const handleChipSelect = (chip: string) => {
    if (!hasRunLanding || isTyping || isRunningAct1) return;
    ...
    <section className="relative flex flex-1 flex-col overflow-hidden">
      ...
      <div className="sticky bottom-0 w-full border-t border-border bg-background/80 backdrop-blur">
        <div className="mx-auto w-full max-w-4xl px-4 md:px-6 pt-2 pb-3 space-y-3">
          <div className="flex flex-wrap items-center gap-3 overflow-x-auto no-scrollbar pb-1 pt-1">
            {suggestionChips.map((chip) => (
              <button
                key={chip}
                type="button"
                onClick={() => handleChipSelect(chip)}
                className="pointer-events-auto rounded-full border border-border px-5 py-2.5 text-[15px] font-semibold text-muted-foreground hover:border-foreground transition-colors shadow-sm"
              >
                {chip}
              </button>
            ))}
