    ...
    <section className="relative flex flex-1 flex-col overflow-hidden">
      ...
      <div className="pointer-events-none fixed inset-x-0 bottom-3 z-30">
        <div className="relative mx-auto flex w-full max-w-[50rem] flex-col gap-2 px-4 md:px-6">
          ...
          <div className="chip-scroll-hint pointer-events-auto mx-auto flex w-full max-w-3xl flex-nowrap items-center gap-2 overflow-x-auto px-0.5 pb-1 no-scrollbar">
            {activeChips.map((chip, index) => (
              <button
                ...
                disabled={isTyping || isRunningAct1 || isActionLimitReached}
                className={`pointer-events-auto shrink-0 rounded-xl border border-white/20 bg-[hsl(var(--background)/0.82)] px-3 py-2 text-sm font-medium text-foreground/90 shadow-[0_8px_20px_-16px_hsl(var(--accent))] backdrop-blur transition duration-200 hover:-translate-y-0.5 hover:border-[hsl(var(--accent)/0.55)] hover:bg-[hsl(var(--muted)/0.75)] hover:text-foreground hover:shadow-[0_12px_24px_-14px_hsl(var(--accent)/0.7)] disabled:cursor-not-allowed disabled:opacity-50 ${animateAct1Chips ? "fade-in" : ""}`}
