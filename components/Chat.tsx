...
export default function Chat({
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // 1) Render assistant text (if any)
      const assistantText =
        (typeof data?.text === "string" && data.text.trim()) || "OK.";
      ...
      if (tool && tool.name === "ui_show_videos") {
        const ids: unknown = tool?.args?.videoIds;
        const validIds = Array.isArray(ids) ? ids.filter((x) => typeof x === "string") : [];
        if (validIds.length) {
          if (onShowVideo) onShowVideo(validIds);
          else if ((globalThis as any).uiTool?.show_videos) {
            (globalThis as any).uiTool.show_videos(validIds);
          }
        }
      }
    } catch {
