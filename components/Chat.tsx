...
export default function Chat({
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: "m0", role: "assistant", text: intro, isNudge: false },
  ]);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Mark as nudge-highlightable iff we triggered a nudge this turn
      push("assistant", reply, !!nudge);
    ...
    (globalThis as any).dispatchLLMEvent = async (evt: { type: string; id?: string; url?: string }) => {
      if (evt?.type === "video_opened") {
        ...
        try {
          ...
          if (text) {
            // Mark as nudge-highlightable iff this was a nudge
            push("assistant", text, !!nudge);
            ...
            {(() => {
              ...
              if (last?.isNudge && typed) {
                ...
                if (span) {
                  ...
                  // Fallback: accent the whole span (no bold splitting)
                  return (
                    <>
                      {typed.slice(0, span.start)}
