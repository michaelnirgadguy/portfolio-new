    ...
    <section className="relative flex flex-1 flex-col overflow-hidden">
      ...
      <div className="pointer-events-none fixed inset-x-0 bottom-3 z-30">
        <div className="relative mx-auto flex w-full max-w-[50rem] flex-col gap-2 px-4 md:px-6">
          ...
          <div className="chip-scroll-hint pointer-events-auto mx-auto flex w-full max-w-3xl flex-nowrap items-center gap-2 overflow-x-auto pb-1">
            {activeChips.map((chip, index) => (
              <button
                ...
                disabled={isTyping || isRunningAct1 || isActionLimitReached}
                className={`pointer-events-auto shrink-0 rounded-xl border border-white/20 bg-[linear-gradient(180deg,rgba(255,255,255,0.12),rgba(255,255,255,0.05))] px-3 py-2 text-sm font-medium text-foreground/90 shadow-[0_6px_20px_rgba(0,0,0,0.2)] transition-all duration-200 hover:-translate-y-0.5 hover:border-[hsl(var(--accent))] hover:bg-[linear-gradient(180deg,rgba(245,158,11,0.28),rgba(245,158,11,0.1))] hover:text-foreground disabled:cursor-not-allowed disabled:opacity-50 ${animateAct1Chips ? "fade-in" : ""}`}
